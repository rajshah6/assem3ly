...
interface Step {
  ...
  /** Optional step-level transform to apply to all parts (e.g., flip entire assembly) */
  groupTransform?: {
    position?: { x: number; y: number; z: number };
    rotation?: { x: number; y: number; z: number };
    scale?: { x: number; y: number; z: number };
...
export function CumulativeScene({
  ...
  const getAnimationsForPart = (partId: string) => {
    ...
    return currentStepData.assemblySequence.filter(
      (anim) => anim.targetId === partId
    );
  };
  ...
  const getStartDelay = (partId: string) => {
    ...
    const partIndex = currentStepData.assemblySequence.findIndex(
      (anim) => anim.targetId === partId
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Render ALL accumulated parts, optionally wrapped in a step-level transform */}
        {(() => {
          const t = currentStepData.groupTransform || {};
