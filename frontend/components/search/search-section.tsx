  ...
  useEffect(() => {
    const handle = setTimeout(async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const data = await searchManuals(currentQuery);
        setResults(data);
      } catch (e) {
    ...
    <Card className="p-5">
      {!hideInput && (
        <div className="flex items-center justify-between gap-3">
          ...
          <Button className="mt-7" onClick={() => (onExternalQueryChange ? onExternalQueryChange(externalQuery ?? query) : setQuery(query))} disabled={loading}>
            {loading ? (
